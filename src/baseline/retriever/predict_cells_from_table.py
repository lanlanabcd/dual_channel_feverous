...
def convert_evidence_into_tables(annotation, db, args):
    ...
    evidence = set(list(predicted_sentences[:2] + predicted_tables[:2]))
...
def tokenize_and_align_labels(text_in, labels_in,  all_input_ids, tokenizer):
    ...
    ori_all_input_ids = all_input_ids
    span = 1000
    ...
    for stat in tqdm(range(0, len(ori_text_in), span)):
        ...
        # key, value 有用
    return all_tokenized_inputs, all_input_ids_new, all_labels
...
def extract_cells_from_tables(annotations, args):
 ...
 #       anno_ids += len(tabs) * [anno.get_id()]
    ...
    if not args.trivial_baseline:
        ...
        all_input_ids = [[pred for i, pred in enumerate(label) if labels[j][i] != -100] for j, label in
                         enumerate(all_input_ids)]


        predictions = (model_output.predictions > 0.25).astype(int)
        # print(predictions)
        predictions = [[1 if ele[1] == 1 else 0 for ele in e] for e in predictions]
        predictions = [[pred for i, pred in enumerate(pred_instance) if labels[j][i] != -100] for j, pred_instance in
                       enumerate(predictions)]
        # print(predictions)
        predictions_map = {}
        for i,annota in enumerate(anno_ids):
            select_cells = [all_input_ids[i][j] for j,el in enumerate(predictions[i]) if predictions[i][j] == 1 and all_input_ids[i][j] != 0]
            # select_cells = list(set(select_cells))
            if not anno_ids[i] in predictions_map:
                predictions_map[anno_ids[i]] = select_cells
            else:
                predictions_map[anno_ids[i]].extend(select_cells)
        # predictions_map = {anno_ids[i]:
        #                        [all_input_ids[i][j] for j,el in enumerate(predictions[i]) if predictions[i][j] == 1 and all_input_ids[i][j] != 0] for i,annota in enumerate(anno_ids)}

        # predictions = torch.log_softmax(torch.tensor(model_output.predictions), dim=-1)[:, :, 0].numpy()
        #
        # predictions = [[pred for i, pred in enumerate(pred_instance) if labels[j][i] != -100] for j,pred_instance in enumerate(predictions)]
        #
        # predictions_map = {anno_ids[i]: [(predictions[i][j], all_input_ids[i][j]) for j, el in enumerate(predictions[i]) if all_input_ids[i][j] != 0] for i, annota in enumerate(anno_ids)}
        #
        # predictions_map = {pm: [p[1] for p in sorted(predictions_map[pm], reverse=True)[:25]] for pm in predictions_map}
    ...
    with jsonlines.open(os.path.join(args.input_path.split('.jsonl')[0] + '.cells.jsonl'), 'w') as writer:
        with jsonlines.open(os.path.join(args.input_path)) as f:
             for i,line in enumerate(f.iter()):
                 ...
                 line['predicted_evidence'] += predictions_map.get(i, [])[:25]
                 writer.write(line)
