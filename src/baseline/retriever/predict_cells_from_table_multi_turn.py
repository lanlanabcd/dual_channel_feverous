    ...
    for tn in range(turn_num):
     ...
     #       anno_ids += len(tabs) * [anno.get_id()]
        ...
        for k, v in predictions_map.items():
            if k in merged_prediction_map:
                merged_prediction_map[k].extend(v)
            else:
                merged_prediction_map[k] = v
    predictions_map = merged_prediction_map
